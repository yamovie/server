...
module.exports.mpaa = mpaa;

module.exports.getCertification = cert => {
  for (let i = 0; i < mpaa.length; i++) {
    if (mpaa[i].certification === cert) return mpaa[i];
  }

  return [];
};

module.exports.filterCertifications = order => {
  return mpaa
    .filter(elem => elem.order <= order)
    .map(elem => elem.certification);
};
